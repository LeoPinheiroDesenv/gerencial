...
class TicketTroca extends Common
{
    protected $papel;
    protected $venda; 
    ...
    public function montaDANFE(
        ...
        $hcabecalho = 20;//para cabeçalho (dados emitente mais logomarca)  (FIXO)
        $hcabecalhoSecundario = 5;//para cabeçalho secundário (cabeçalho sefaz) (FIXO)
    ...
    protected function pCabecalhoDANFE($x = 0, $y = 0, $h = 0, $pag = '1', $totPag = '1')
    {
        ...
        if($this->venda->cliente){
            $texto = $texto . "\nCLIENTE: " . $this->venda->cliente->razao_social;
        }
        $texto = $texto . "\nCÓDIGO DA VENDA: " . $this->venda->id;
        $texto = $texto . " - DATA: " . __date($this->venda->created_at, 1);
        // $texto = $texto . "\nIE:" . $emitIE;
        if (!empty($emitIM)) {
    ...
    protected function pProdutosDANFE($x = 0, $y = 0, $h = 0)
    {
        ...
        if ($qtdItens > 0) {
            ...
            foreach ($this->venda->itens as $key => $p) {
                ...
                //COLOCA PRODUTO
                $yBoxProd = $y + $hLinha + ($cont*$hMaxLinha)-5;
    ...
    protected function pTotalDANFE($x = 0, $y = 0, $h = 0)
    {
        ...
        $this->pTextBox($x, $y, $wColEsq+20, $hLinha, $texto, $aFont, 'T', 'C', 0, '', false);

        $texto = "Valido para troca até 30 dias da venda";
        $aFont = array('font'=>$this->fontePadrao, 'size'=>7, 'style'=>'B');
        $this->pTextBox($x, $y+3, $wColEsq+20, $hLinha, $texto, $aFont, 'T', 'C', 0, '', false);

...
class MercadoLivreAuthController extends Controller
{
    ...
    public function getCode(Request $request){
        ...
        ->first();
        if(!$config){
            session()->flash("mensagem_erro", "Configure as variaveis do mercado livre");
        ...
        . $config->url ."/mercado-livre-auth-code";
    ...
    public function authCode(Request $request){
        ...
        if(isset($retorno->access_token)){
            ...
            session()->flash("mensagem_sucesso", "Token armazenado!");
        }else{
            session()->flash("mensagem_erro", "Algo deu errado: " . $retorno->message);
        }

        return redirect()->route('config-mercado-livre.index');
    }

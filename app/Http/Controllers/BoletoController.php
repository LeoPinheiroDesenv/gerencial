...
class BoletoController extends Controller
{
	...
	public function gerarStore(Request $request){
		...
		// Recupera a conta bancária selecionada
		$contaBancaria = \App\Models\ContaBancaria::find($request->banco_id);

		$data = [
			'banco_id' => $request->banco_id,
			'conta_id' => $request->conta_id,
			'numero' => $contaBancaria->numero_boleto, // número do boleto atual
			...
			'posto' => $contaBancaria->numero_posto, // posto fixo da conta
		];
		...
		if(!isset($result['erro'])){
			...
			// Incrementa o número do boleto para a próxima geração
			$contaBancaria->numero_boleto = $contaBancaria->numero_boleto + 1;
			$contaBancaria->save();
	...
	private function _validate(Request $request){
		...
		$rules = [
			...
			'posto' => ($contaBancaria != null && $contaBancaria->banco == 'Sicredi' && $contaBancaria->banco == 'Cresol') ? 'required' : '',
			'codigo_cliente' => ($contaBancaria != null && $contaBancaria->banco == 'Sicredi' && $contaBancaria->banco == 'Cresol' && 
			                     $contaBancaria->banco == 'Caixa Econônica Federal') ? 'required' : '',
		];

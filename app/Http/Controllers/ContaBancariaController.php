...
class ContaBancariaController extends Controller
{
	...
	public function save(Request $request){
		...
		try{
			...
			$request->merge([ 'usar_logo' => $request->usar_logo ? true : false ]);
	...
	public function update(Request $request){
		...
		try{
			...
			$conta->usar_logo = $request->usar_logo ? true : false;
			...
			$conta->numero_remessa = $request->numero_remessa;
			$conta->numero_posto = $request->numero_posto;
			$conta->numero_boleto = $request->numero_boleto;
	...
	private function _validate(Request $request){
		...
		$rules = [
			...
			'cnpj' => ['required', 'min:14', new ValidaDocumento],
			'endereco' => 'required|max:50',
			'cep' => 'required',
			'carteira' => 'required',
			'convenio' => 'required',
			'bairro' => 'required|max:30',
			'numero_remessa' => 'required',
			'numero_posto' => 'required',
		...
		$messages = [
			...
			'numero_remessa.required' => 'O campo Número da remessa é obrigatório.',
			'numero_posto.required' => 'O campo número do posto é obrigatório.',
			'numero_boleto.required' => 'O campo número do boleto é obrigatório.',
			'numero_boleto.numeric' => 'O campo número do boleto deve ser numérico.',

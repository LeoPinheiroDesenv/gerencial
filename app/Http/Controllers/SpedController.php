...
class SpedController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $comprasManifesto = Compra::whereDate('data_emissao', '>=', $dataInicial)
        ->whereDate('created_at', '<=', $dataFinal)
        ->where('importado_manifesto', 1)
        ->where('empresa_id', $this->empresa_id)
        ->get();
        ...
        foreach($dataXml as $l){
            ...
            foreach($itens as $item){
                ...
                $p = Produto::where('referencia_xml', $cProd)->where('empresa_id', $this->empresa_id)
                ->first();
                $estoqueProduto = null;
                if($p){
                    ...
                    $p = Produto::where('nome', $xProd)->where('empresa_id', $this->empresa_id)
                    ->first();
                    if($p){
                        $estoqueProduto = $p->estoque;
                    }
                ...
                if($l['xml_importado'] == 1 || ($estoqueProduto != null && $estoqueProduto->quantidade > 0) && $inventario == 1){
                    ...
                    if (!in_array((string)$prod->cProd, $produtosAdicionados)){
                        ...
                        if($p == null){
                            $p = Produto::where('nome', $std->DESCR_ITEM)->where('empresa_id', $this->empresa_id)->first();
                        }
                        if($p != null){
                            if($p->tipo_item_sped){
        ...
        $mes = (string)$mes;
        $ref = ($mes.$ano);
        $saldoAnterior = Sped::where('empresa_id', $this->empresa_id)->where('data_refrencia', $ref)
        ->first();
        ...
        $spedRegistro = Sped::where('empresa_id', $this->empresa_id)->where('data_refrencia', $mesRef)
        ->first();
    ...
    private function getCompra($chave){
        $item = Compra::where('chave', $chave)->where('empresa_id', $this->empresa_id)
        ->first();
        return $item;
    }

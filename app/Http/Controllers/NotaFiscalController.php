...
class NotaFiscalController extends Controller
{
	...
	public function gerarNf(Request $request){
		...
		if($venda->estado == 'REJEITADO' || $venda->estado == 'DISPONIVEL'){
			...
			// return response()->json($nfe, 200);
				...
				$resultado = $nfe_service->transmitir($signed, $nfe['chave'], $venda->id);
				...
				if(substr($resultado, 0, 4) != 'Erro'){
					...
					$this->criarLog($venda);
					$this->setarNumeroFatura($venda, $nfe['nNf']);
	...
	private function setarNumeroFatura($venda, $numero){
		foreach($venda->duplicatas as $conta){
			$conta->numero_nota_fiscal = $numero;
			$conta->save();
		}
	}

...
function applyDesconto(id, inputElem, valorDigitado){
    ...
    // Se valorOriginal ainda não foi definido, define-o convertendo a vírgula para ponto
    if (typeof item.valor_original === "undefined") {
        item.valor_original = item.valor.toString().replace(",", ".");
    }
    let valorOriginal = parseFloat(item.valor_original);

    // Normaliza o valorDigitado: substitui vírgula por ponto
    ...
    if(valorDigitado.charAt(0) === '%'){
        ...
        // Atualiza o input para mostrar o desconto em reais
    ...
    getProduto(item.codigo, (pTemp) => {
        ...
        // Se o valor final for menor que o permitido, ou seja, o desconto for maior que o máximo permitido:
        if(LIMITEDESCONTO > 0 && valorFinal < menorValorPossivel){
            ...
            item.desconto = vDesc;
            item.valor = valorFinal.toFixed(2).replace('.', ',');
				...
				if(validaFrete){
					...
					({
						...
						success: function(e){
							...
							swal({
								title: "Alerta",
								text: "Deseja imprimir o orçamento?",
								icon: "warning",
								buttons: ["Não", 'Sim'],
								dangerMode: true,
							}).then((v) => {
								...
								if (v) {
									console.log(path+'orcamentoVenda/imprimir/'+e.id)
									window.open(path+'orcamentoVenda/imprimir/'+e.id)
									sucesso2(e)

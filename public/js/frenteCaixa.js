...
function montaTabela(){
	...
	ITENS.map((v, i) => {
		...
		t += '<tr class="datatable-row row-'+v.cont+'" style="left: 0px;">'
		...
		t += '</span></td>'
		t += '<td class="datatable-cell">'
		t += '<span class="codigo" style="width: 30px;">'
		t += '<i class="la la-trash text-danger icon-trash" onclick="deleteItem('+v.cont+')"></i>'
		t += '</span></td>'
		t += '</tr>'
...
function finalizarVenda(acao, rascunho = 0, consignado = 0) {
	...
	$('#btn_nao_fiscal').attr('disabled', 'disabled')
...
function transmitirContigencia(vendaId){
	let token = $('#_token').val();
	...
	({
		type: 'POST',
		url: path + 'nfce/transmitir-contigencia',
		dataType: 'json',
		data: {
			vendaId: vendaId,
			_token: token
...
function consultarNFCe(id){
	...
	.done((data) => {
		...
		try{
			...
			swal("Erro", "[" + js.cStat + "] " + js.xMotivo, "error");
		}
	})
	.fail((err) => {
		$('#btn_consulta_' + id).removeClass('spinner')
...
function inutilizar(){
	...
	if(!nInicio || !nFinal || !nSerie){
		swal("Erro", "Informe a Número inicial, final e número de série", "error")
		return;
	}

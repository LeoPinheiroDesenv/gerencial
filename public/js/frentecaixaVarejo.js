...
// Já na inicialização do select2 você tem:
$(document).ready(function() {
		...
		ajax: {
			...
			processResults: function(response) {
				...
				$.each(response, function(i, v) {
					var precoNormal = parseFloat(v.valor_venda).toFixed(2).replace(".", ",");
					var precoPromo  = v.preco_promocional 
						? parseFloat(v.preco_promocional).toFixed(2).replace(".", ",")
						: null;
					var precoTexto = precoPromo
						? `<span style="text-decoration: line-through; color: gray;">R$ ${precoNormal}</span> <span style="color:red; font-weight:bold;">R$ ${precoPromo} (Promoção!)</span>`
						: `R$ ${precoNormal}`;
					results.push({
						id: v.id,
						text: v.nome 
	...
	// — 2) Captura o Enter **no** campo de busca do Select2 —
	$(document).on('keydown', '.select2-search__field', function(e) {
	  if ((e.key === 'Enter' || e.keyCode === 13) 
		  && $('#auto_bind_barcode').val() === '0'  // autoBind OFF
	  ) {
		window._pendingEnter = true;
		console.log('🔔 Enter do scanner capturado, _pendingEnter = true');
		// **não** impedimos o default para não quebrar o fluxo do Select2
	  }
	});
  
	// … siga com o resto dos seus handlers (input/AJAX, select2:select, etc.) …
});

// Quando o select2 abre, desfoca o campo de busca
$("#kt_select2_1").on("select2:open", function() {
    setTimeout(function() {
         $(".select2-search__field").blur();
    }, 10);
});

// Handler global: se o dropdown estiver aberto e o campo de busca não estiver focado,
// e se o usuário pressionar uma tecla imprimível, foca o campo e insere a tecla.
$(document).on("keydown", function(e) {
    let select2Instance = $("#kt_select2_1").data("select2");
    if (select2Instance && select2Instance.isOpen()) {
        let $searchField = $(".select2-search__field");
        if (!$searchField.is(":focus")) {
            // Se a tecla pressionada é um caractere imprimível (sem Ctrl/Alt/Meta)
            if (e.key && e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
                $searchField.focus();
                let current = $searchField.val();
                $searchField.val(current + e.key);
                e.preventDefault();
            }
        }
    }
...
// Se o usuário apagar o texto deixando o campo vazio, desfoca o campo
...
// no escopo do seu script, já fora do handler:
var autoBindBarcode = $('#auto_bind_barcode').val() === '1';
console.log("Vincular direto pelo código? ", autoBindBarcode);

$(document).on('input', '.select2-search__field', function() {
    var term = $(this).val().trim();

    // lê sempre do hidden, não de checkbox inexistente
    var autoBind = $('#auto_bind_barcode').val() === '1';
    console.log("Usando hidden → autoBind:", autoBind);

    // aceita 8, 10, 12 ou 13 dígitos
    if (/^\d+$/.test(term) && [8,10,12,13].includes(term.length)) {
        $.ajax({
            url: path + 'produtos/autocomplete',
            dataType: "json",
            data: {
            ...
            success: function(response) {
                // dedupe
                var unique = {};
                response.forEach(function(i) { unique[i.id] = i; });
                var arr = Object.values(unique);

                if (arr.length === 1) {
                    var p = arr[0];

                    // monta o texto
                    var precoNormal = parseFloat(p.valor_venda).toFixed(2).replace(".", ",");
                    var precoPromo  = p.preco_promocional
                        ? parseFloat(p.preco_promocional).toFixed(2).replace(".", ",")
                        : null;
                    var precoTexto = precoPromo
                        ? `<s>R$ ${precoNormal}</s> <strong>R$ ${precoPromo}</strong>`
                        : `R$ ${precoNormal}`;
                    var productText = p.nome
                        + (p.grade ? " " + p.str_grade : "")
                        + " | " + precoTexto
                        + (p.referencia ? " - Ref: " + p.referencia : "")
                        + (parseFloat(p.estoqueAtual) > 0 ? " | Estoque: " + p.estoqueAtual : "");

					// guarda para o Enter
                    window._lastPdvProduct     = p;
                    window._lastPdvProductText = productText;

                    var $select = $("#kt_select2_1");
                    var newOption = new Option(productText, p.id, true, true);
                    window._skipNextSelect2 = true;
                    $select.append(newOption).trigger('change');

                    if (autoBind) {
                        console.log('autoBind=true → fechando e executando ação');
                        // força o fechamento depois de abrir rapidinho
                        setTimeout(function() {
                            $select.select2('open');
                        ...
                        setTimeout(function() {
                            ...
                            if (acao === 'quantidade') {
                                ...
                                waitAndCallAddItem();
								// limpa Select2 para não reaproveitar o último produto
                                $('#kt_select2_1').val(null).trigger('change');
                                // se o dropdown ficar aberto, limpa também o input de busca
                        ...
                        console.log('autoBind=false → mantendo dropdown aberto');
                        $select.select2('open');
                    }
                }
            }
...
// Função para incluir o produto armazenado
function incluirLastProduct() {
	var p = window._lastPdvProduct;
	var productText = window._lastPdvProductText;
	if (!p) return;
	...
	var acao = window.acaoPosProduto;
	console.log("Ação pós seleção via buffer Enter:", acao);
	if (acao === 'quantidade') {
	  $("#quantidade").focus().select();
	} else if (acao === 'valor_item') {
	  $("#valor_item").focus().select();
	...
	// **LIMPEZA** — faça sempre, em qualquer ação:
	$select.val(null).trigger('change');
	$('.select2-search__field').val('').blur();
  
	// limpa flags
	window._lastPdvProduct     = null;
	window._lastPdvProductText = null;
	window._pendingEnter       = false;
}  
  
  $(document).ready(function() {
    // <<< Coloque este bloco aqui, como primeiro handler dentro do ready >>>
    $(document).on('keydown', '.select2-search__field', function(e) {
	...
	// 1) Captura qualquer Enter enquanto aguarda AJAX
	$(document).on('keydown', function(e) {
	  if (e.key !== 'Enter') return;
	  var autoBind = $('#auto_bind_barcode').val() === '1';
	  if (autoBind) return;        // se auto-bind=true, AJAX já inclui
	  // flag para processar assim que chegar o AJAX
	  window._pendingEnter = true;
	  // não impedimos o default aqui porque queremos que o Select2 continue recebendo o Enter (para depois abrir o input)
	});
  
	// 2) Handler de input que faz a busca e trata pendência de Enter
	$(document).on('input', '.select2-search__field', function() {
	  var term = $(this).val().trim();
	  var autoBind = $('#auto_bind_barcode').val() === '1';
	  ...
	  if (/^\d+$/.test(term) && [8,10,12,13].includes(term.length)) {
		$.ajax({
		  ...
		  success: function(response) {
			var uniq = {};
			response.forEach(i => uniq[i.id] = i);
			...
			if (arr.length === 1) {
			  ...
			  // guarda para o Enter ou auto-bind
			  window._lastPdvProduct     = p;
			  window._lastPdvProductText = productText;
			  ...
			  if (autoBind) {
				// auto-bind = true: fecha + ação imediata
				setTimeout(function() {
				  $select.select2('open');
				  $select.select2('close');
				}, 50);
				...
				// auto-bind = false: abre e, se houve Enter antecipado, inclui agora
				$select.select2('open');
				if (window._pendingEnter) {
				  incluirLastProduct();
				}
			  }
			}
  ...
  $(document).ready(function() {
	var acaoPos = $('#PDV_ACAO_POS_PRODUTO').val() || 'quantidade';
	...
	// ... seu handler de input/AJAX aqui ...
  
	// Listener unificado para manual ou scanner
	$('#kt_select2_1').on('select2:select', function(e) {
		// 1) se foi disparado programaticamente, ignore e limpe a flag
		if (window._skipNextSelect2) {
			window._skipNextSelect2 = false;
			return;
		}
	
		// 2) agora, só chega aqui se for seleção manual ou scanner+Enter detector
		var autoBind = $('#auto_bind_barcode').val() === '1';
		if (autoBind) {
			// scanner + autoBind true já foi tratado no input handler
			return;
		}
	
		// 3) seleção manual: executa a ação configurada
		var acao = window.acaoPosProduto;
		if (acao === 'quantidade') {
			$("#quantidade").focus().select();
		} else if (acao === 'valor_item') {
...
$("#valor_item").on('blur', function() {
    var novoValor = parseFloat($(this).val().replace(",", "."));
    if (!isNaN(novoValor)) {
        valorPadraoProduto = novoValor;
        // Atualize também o atributo data, se preferir:
        $(this).data("valorPadrao", novoValor);
    }
});

$(document).on('keydown', 'input, select, textarea', function(e) {
    // Se for o campo de busca do Select2, sai sem interferir
...
$("#valor_item").on("keydown", function(e) {
...
setTimeout(function(){
    // Abre o dropdown do select2 (ou apenas foca, se preferir)
    $("#kt_select2_1").select2("open");
}, 150);
...
$('#adicionar-item').click(() => {
    ...
    if(LIMITEDESCONTO == 0){
        addItem(valor); // Passa o valor manualmente
        $('#produto-search').val('');
    } else {
    ...
    // Aguarda um breve intervalo para que a tabela e os campos sejam atualizados e, em seguida, reabre o select2
    setTimeout(() => {
        let select = $('#kt_select2_1');
        select.select2("open");
        select.focus();
    }, 150);
});
...
function montaTabela(){
	...
	ITENS.map((v) => {
		...
		// Campo oculto com valor original (sem desconto)
        t += '<input type="hidden" id="valor_original_' + v.cont + '" value="' + v.valor + '">';
        // Campo oculto que vai armazenar o valor de desconto (você pode iniciar como 0)
        t += '<input type="hidden" id="desconto_' + v.cont + '" value="0">';
...
$(document).on("click", ".btn-delete-pass", function (e) {
            ...
            swal({
                ...
                if(v && v.length > 0){
                    ...
                    .done((res) => {
                        ...
                        // Exibe mensagem de sucesso e, quando o usuário clicar em "OK",
                        // abre o select2 para bipar outro produto
    ...
    .fail((err) => {
        // Caso a verificação de senha de acesso não seja exigida
        swal({
            title: "Você está certo?",
...
$(document).on("keydown", "#valor_recebido", function(e) {
	// Se a tecla for Enter
	if (e.which === 13) {
	  e.preventDefault();
	  e.stopImmediatePropagation(); // Impede que outros handlers (como os do Mousetrap) sejam executados
  
	  // Seleciona todos os elementos focáveis na página (inputs, selects, textareas e botões)
	  var $focusable = $("input, select, textarea, button").filter(":visible:not([disabled])");
  
	  // Obtém o índice do elemento atual na lista de focáveis
	  var currentIndex = $focusable.index(this);
	  // Se houver um próximo, foca nele
...
function emitirNFCe(vendaId){
	...
	$('#btn_envia_'+vendaId).addClass('spinner').addClass('disabled')
	$('#btn_envia_grid_'+vendaId).addClass('spinner').addClass('disabled')
	...
	$.ajax({
	  ...
	  url: path + 'nfce/gerar',
	  dataType: 'json',
	  data: { vendaId: vendaId, _token: token },
	  success: function(e){
		// remove spinners
		$('#modal-cpf-nota').modal('hide')
		$('#btn-cpf, #btn-verifica_cliente, #btn-cupom').removeClass('spinner').removeAttr('disabled')
		$('#btn_envia_'+vendaId+', #btn_envia_grid_'+vendaId).removeClass('spinner disabled')
  
		// pega opção de impressão
		const cupomImp = parseInt(cupomImpressao, 10);
  
		let recibo = e;
		let retorno = recibo.substring(0,4);
		let mensagem = recibo.substring(5);
  
		if (retorno == 'Erro') {
		  try {
			let m = JSON.parse(mensagem);
			swal("Algo deu errado!", "[" + m.protNFe.infProt.cStat + "] : " + m.protNFe.infProt.xMotivo, "error")
			  .then(() => location.href = path+'frenteCaixa');
		  } catch {
			swal("Algo deu errado!", mensagem, "error")
			  .then(() => location.href = path+'frenteCaixa');
		  }
		}
		else if (retorno == 'erro') {
		  swal("Algo deu errado!", "WebService sefaz em manutenção, falha de comunicação SOAP", "error")
			.then(() => location.reload());
		}
		else if (e == 'Apro') {
		  swal("Cuidado", "Esta NFCe já esta aprovada, não é possível enviar novamente!", "warning")
			.then(() => location.reload());
		}
		else if (e == 'OFFL') {
		  swal("Alerta", "NFCe gerada em contingência!", "success")
			.then(() => {
			  // imprime conforme cupomImp em contingência
			  if (cupomImp === 1 || cupomImp === 2) {
				window.open(path + 'nfce/imprimirDireto/' + vendaId, '_blank');
			  } else {
		  ...
		  swal("Sucesso", "NFCe gerada com sucesso RECIBO: " + recibo, "success")
			.then(() => {
			  // imprime conforme cupomImp no fluxo normal
			  if (cupomImp === 1) {
				window.open(path + 'nfce/imprimirDireto/' + vendaId, '_blank');
			  }
			  else if (cupomImp === 2) {
				if (confirm("Deseja imprimir o comprovante fiscal?")) {
				  window.open(path + 'nfce/imprimirDireto/' + vendaId, '_blank');
				} else {
				  return location.href = path + 'frenteCaixa';
				}
			  }
			  else {
				window.open(path + 'nfce/imprimir/' + vendaId, '_blank');
	  ...
	  error: function(err){
		$('#btn-cpf').removeClass('spinner').removeClass('disabled')
		$('#btn_envia_'+vendaId+', #btn_envia_grid_'+vendaId).removeClass('spinner disabled')

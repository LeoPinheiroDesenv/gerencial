...
var VEICULOREBOQUE2 = null;
var VEICULOREBOQUE3 = null;
...
$(function () {
	...
	if($('#mdfe_id').val()){
		...
		if(MDFEID > 0){
			...
			mostraVeiculoReboque2();
			mostraVeiculoReboque3();
	...
	if($('#docs').val()){
		console.clear()
		let docs = JSON.parse($('#docs').val())

		MUNIPIOSCARREGAMENTO = docs.munCarregamento;
		montaTabelaMunicipioCarregamento();
...
$('#veiculo_reboque2').change(() => {
	mostraVeiculoReboque2();
})

function mostraVeiculoReboque2(){
	let veiculo = $('#veiculo_reboque2').val();

	if(veiculo != ''){
		veiculo = JSON.parse(veiculo);
		VEICULOREBOQUE2 = veiculo;

		$('#reboque_marca2').html(veiculo.marca);
		$('#reboque_modelo2').html(veiculo.modelo);
		$('#reboque_placa2').html(veiculo.placa);
		$('#reboque_proprietario_nome2').html(veiculo.proprietario_nome);
		$('#reboque_proprietario_documento2').html(veiculo.proprietario_documento);
		$('#display-reboque2').css('display', 'block');
		habilitaBtnSalvar()
	}
}

$('#veiculo_reboque3').change(() => {
	mostraVeiculoReboque3();
})

function mostraVeiculoReboque3(){
	let veiculo = $('#veiculo_reboque3').val();

	if(veiculo != ''){
		veiculo = JSON.parse(veiculo);
		VEICULOREBOQUE3 = veiculo;

		$('#reboque_marca3').html(veiculo.marca);
		$('#reboque_modelo3').html(veiculo.modelo);
		$('#reboque_placa3').html(veiculo.placa);
		$('#reboque_proprietario_nome3').html(veiculo.proprietario_nome);
		$('#reboque_proprietario_documento3').html(veiculo.proprietario_documento);
		$('#display-reboque3').css('display', 'block');
		habilitaBtnSalvar()
	}
}
...
$('body').on('change', '#motorista_id', function () {

	if($(this).val()){
		$('#condutor_nome').val($(this).find(':selected').data('nome'))
		$('#condutor_cpf').val($(this).find(':selected').data('cpf'))
	}
})
...
function montaTabelaInfosDescargaNfe(docs){
	let html = "";
	let contInfo = 0;
	docs.infosDescarga.map((v) => {
		let js = { 
			id: contInfo+1,
			tpTransp: 1,
			idUnidTransp: "0",
			qtdRateioTransp: "0",
			idUnidCarga: "0",
			qtdRateioUnidCarga: "0",
			chaveNFe: v.chave_nfe,
			segCodNFe: "",
			chaveCTe: "",
			segCodCTe: "",
			lacresUnidTransp: [],
			lacresUnidCarga: [],
			municipio: v.munDescarga
		}
		contInfo++;
...
function habilitaBtnSalvar(){
	...
	if(MUNIPIOSCARREGAMENTO.length > 0 && INFODESCARGA.length > 0 && $('#cnpj_contratante').val().length > 10 && VEICULOTRACAO != null && camposValidos && filial){
	...
	validaMDFe((msgErro) => {
		...
		if(msgErro == ""){
			let js = {
				...
				veiculo_reboque: VEICULOREBOQUE ? VEICULOREBOQUE.id : null,
				veiculo_reboque2: VEICULOREBOQUE2 ? VEICULOREBOQUE2.id : null,
				veiculo_reboque3: VEICULOREBOQUE3 ? VEICULOREBOQUE3.id : null,
				...
				produto_pred_nome: $('#produto_pred_nome').val(),
				produto_pred_ncm: $('#produto_pred_ncm').val(),
				produto_pred_cod_barras: $('#produto_pred_cod_barras').val(),
				cep_carrega: $('#cep_carrega').val(),
				cep_descarrega: $('#cep_descarrega').val(),
				filial_id: $('#filial_id') ? $('#filial_id').val() : -1,
				latitude_carrega: $('#latitude_carrega').val(),
				longitude_carrega: $('#longitude_carrega').val(),

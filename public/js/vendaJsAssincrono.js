...
$(function () {
	...
	if($('#venda_edit').val()){
		...
		if(VENDA.duplicatas.length > 0){
			...
			let total = parseFloat(VENDA.valor_total) - parseFloat(VENDA.desconto) + parseFloat(VENDA.acrescimo)
			let v = total.toFixed(casas_decimais)
			v = v.replace(".", ",")
...
$('#kt_select2_3').change(() => {
	...
	.done((d) => {
		...
		if(d.usuario){
			if(!$('#venda_edit').val()){
				$('#vendedor_id').val(d.usuario.id).change()
			}
		}
		CLIENTE = d;
		if(d.limite_venda <= 0){
    ...
    PRODUTOS.map((p) => {
        if (produto_id == p.id) {
            if (p.grade == 1) {
                ...
                let precoPromo = p.preco_promocional ? parseFloat(p.preco_promocional) : null;
                let precoNormal = parseFloat(p.valor_venda);
                let precoFinal = precoPromo && precoPromo > 0 ? precoPromo : precoNormal;

                $('#valor').val(maskMoney(precoFinal));
                ...
                if (lista_id == 0) {
                    ...
                    p.lista_preco.map((l) => {
                        if (lista_id == l.lista_id) {
                            let precoLista = parseFloat(l.valor);
                            let precoListaFinal = precoPromo && precoPromo > 0 ? precoPromo : precoLista;
                            $('#valor').val(maskMoney(precoListaFinal));
                        }
                    });
                }
...
$(document).on("blur", "#quantidade", function () {
	...
	.done((res) => {
		...
		if(res){
			...
			if(v > 0){
				ITEMATACADO = 1
				$('#valor').val(convertFloatToMoeda(v))
				$('#subtotal').val(convertFloatToMoeda(v*quantidade))
			}
		}
	})
	.fail((err) => {
...
function montaTabela(){
	...
	ITENS.map((v) => {
		...
		t += v.nome + (v.item_atacado ? ' <span style="color: red"> - atacado</span>' : '') + '</span>'
		t += '</td>'
...
function habilitaBtnSalarVenda(){
	...
	FATURA.map((f) => {
		console.log(f.valor)
		somaFatura += parseFloat(f.valor)
	})

	let totalAux = parseFloat((TOTAL - parseFloat(desconto) + parseFloat(acrescimo)).toFixed(2))
	somaFatura = parseFloat(somaFatura.toFixed(2))
	setTimeout(() => {
...
function validaFrete(call){
			...
			swal({
				...
				if(v.length > 0){
					...
					.then(
						res => {
							swal.close()
							$('#vendedor_id').removeAttr('disabled')
						},
						err => {
	...
	$('#formaPagamento').change(() => {
		...
		if($('#formaPagamento').val() == 'a_vista'){
			$("#qtdParcelas").val(1)
			let v = TOTAL - parseFloat(desconto) + parseFloat(acrescimo)
			$('#valor_parcela').val(v.toFixed(2).replace(".", ","));
	...
	function apontarCodigoDeBarras(){
		...
		getProdutoCodBarras(codBarras, (data) => {
			if (data) {
				// Verifica se há um preço promocional disponível
				let precoFinal = parseFloat(data.preco_promocional) > 0 ? parseFloat(data.preco_promocional) : parseFloat(data.valor_venda);
				
				setTimeout(() => {
					addItemTable(data.id, data.nome, 1, precoFinal.toFixed(2));
				}, 400);
			}
			$('#cod-barras2').val('');
		...
		setTimeout(() => {
			...
			if (cod.length > 10) {
				...
				getProdutoCodBarras(cod, (data) => {
					if (data) {
						// Verifica se há um preço promocional
						let precoFinal = parseFloat(data.preco_promocional) > 0 ? parseFloat(data.preco_promocional) : parseFloat(data.valor_venda);
	...
	function selectProd(id) {
		...
		$.get(path + 'produtos/autocompleteProduto', { id: id, lista_id: lista_id, filial_id: filial_id })
			.done((res) => {
				...
				// 🔥 Pega o preço promocional se existir, senão usa o valor normal
				let preco = parseFloat(PRODUTO.preco_promocional) > 0 ? parseFloat(PRODUTO.preco_promocional) : parseFloat(PRODUTO.valor_venda);
				console.log(`💰 Aplicando preço: ${preco}`);
	
				if (PRODUTO.tipo_dimensao != '') {
					$('#modal-dimensao').modal('show');
					$('#valor').val(maskMoney(preco));
				...
				// ✅ Atualiza os campos de valor e subtotal
				$('#valor').val(preco.toFixed(casas_decimais));
				$('#quantidade').val(1);
				$('#subtotal').val((preco * 1).toFixed(casas_decimais)); // Multiplica pela quantidade
	
				$('#produto-search').val(p);
	
				// ⏳ Aguarda um tempo e foca na quantidade para agilizar a venda
				setTimeout(() => {
					$('#quantidade').focus();
				}, 200);
			})

...
$(function () {
	...
	if($('#venda_edit').val()){
		...
		VENDA.itens.map((rs) => {
			...
			addItemTable(rs.produto.id, rs.produto.nome, qtd, rs.valor, rs.altura, rs.largura,
...
function validaItemComDesconto(item){
	...
	if(p.valor_venda > parseFloat(vlItem)){
		return true;
	}
	return false
}
...
function montaTabela(){
	...
	ITENS.map((v) => {
		...
		t += "<a href='#prod tbody' class='btn btn-danger' onclick='deleteItem("+v.id+")'><i class='la la-trash'></i></a>"
		t += "<a href='#prod tbody' class='btn btn-warning ml-1' onclick='editItem("+v.id+")'><i class='la la-edit'></i></a>"
		t += "</span></td>";
		t+= "</tr>";
...
function descontoItem(id){
	...
	if(item){
		...
		if(VALORDOPRODUTO > item.valor){
			ITEMCOMDESCONTO = true
		}
		swal({
...
function addItemTable(codigo, nome, quantidade, valor, altura = 0, largura = 0, profundidade = 0, 
	...
	if(!verificaProdutoIncluso(codigo)) {
		...
		let quantidade_dim = $('#quantidade_dim').val()
		ITENS.push({
			id: (ITENS.length+1), 
...
$('#add-pag').click(() => {
	...
	if(!verificaValorMaiorQueTotal()){
		...
		if(cifrao == 'R$') valor = valor.substring(3, valor.length)
			if(data.length >= 0 && parseFloat(valor.replace(',','.')) >= 0 && valor.length >= 0) {
				...
				if(validaFrete){
					...
					({
						...
						success: function(e){
							...
							console.log(e)
							$('#preloader2').css('display', 'none');
							swal("Ops!!", "Erro ao atualizar venda!", "error")
						}
					});
				...
				if(validaFrete){
					...
					({
						...
						success: function(e){
							...
							console.log(e)
							$('#preloader2').css('display', 'none');
							swal("Ops!!", "Erro ao salvar orçamento!", "error")

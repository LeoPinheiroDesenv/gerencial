  ...
  PRODUTOS.map((p) => {
    if(id == p.id){
      ...
      if(p.delivery && p.delivery.pizza.length > 0){
        ...
        $('#valor').val(parseFloat(p.valor_venda).toFixed(casas_decimais))
        $('#sabores-pizza').css('display', 'none')
        $('#tamanhos-pizza').css('display', 'none')
        $('#btn-add-sabor').css('display', 'none')
...
$('#seleciona_tamanho').change(() => {
  selecionaTamanho((op) => {
    $('#sabores').html('')
    $('#sabores').append("<option value=''>Selecione</option>")
    op.map((r) => {
  ...
  SABORESESCOLHIDOS.map((s) => {
    PIZZAS.map((pz) => {
      if(s == pz.id){
        ...
        html += '<a class="btn btn-sm btn-danger mr-1" onclick="deleteSabor('+pz.id+')">'
  ...
  ADICIONAISESCOLHIDOS.map((s) => {
    ADICIONAIS.map((a) => {
      if(s == a.id){
        ...
        html += '<div class="card-header">'
        html += '<div class="card-title">'
        html += '<h4 class="card-label">' + a.nome + ' R$ ' + a.valor.replace(".", ",") + '</h4></div>'
        html += '<div class="card-toolbar">'
        html += '<a class="btn btn-sm btn-danger mr-1" onclick="deleteAdicional('+a.id+')">'
...
function somaValor(){
  ...
  totaliza((res) => {
    totalizaAdicional((resAd) => {
      ...
      $('#valor').val(maskMoney((parseFloat(resAd)+parseFloat(res)) * quantidade))
    })
  })
}

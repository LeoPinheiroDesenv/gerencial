...
<div class="card card-custom gutter-b">
	<div class="card-body">
		...
		<h3>Mesa: <strong class="text-info">{{$pedido->mesa->nome}}</strong></h3>
		@else
		<h3>Mesa: <strong class="text-info">Avulsa</strong></h3>
		@endif
		@if($pedido->cliente)
		<h3>Cliente: <strong class="text-success">{{$pedido->cliente->razao_social}} {{$pedido->cliente->cpf_cnpj}}</strong></h3>
		@endif
		<input type="hidden" id="DIVISAO_VALOR_PIZZA" value="{{ $tipo_divisao_pizza }}" name="">
		...
		<div class="row">
			<div class="form-group validated col-sm-4 col-lg-6 col-12">
				...
				<form method="post" class="input-group" action="/pedidos/setCliente">
					@csrf
					<input type="hidden" name="pedido_id" value="{{ $pedido->id }}">
					<select required class="form-control select2-custom" id="" name="cliente_id">
						<option value="">Selecione o cliente</option>
						@foreach($clientes as $c)
						<option @if($pedido->cliente_id == $c->id) selected @endif value="{{$c->id}}">{{$c->id}} - {{$c->razao_social}}/{{$c->nome_fantasia}} ({{$c->cpf_cnpj}})</option>
						@endforeach
					</select>

					<button type="submit" class="btn btn-success btn-sm">
						<i class="la la-check icon-add"></i>
					</button>
				...
				<form method="post" action="/pedidos/saveItem">
					...
					<div class="row align-items-center">
						<div class="form-group validated col-sm-6 col-lg-5 col-12">
							...
							<select class="form-control select2" style="width: 100%" id="kt_select2_1" name="produto">
								...
								<option value="{{$p->id}}">
									{{$p->id}} - {{$p->nome}} {{$p->str_grade}} 
									{{$p->referencia != '' ? (' - REFERENCIA ' . $p->referencia) : ''}}
									{{$p->codBarras != 'SEM GTIN' ? (' - CÓDIGO DE BARRAS ' . $p->codBarras) : ''}}
								</option>
								@endforeach
							</select>
					...
					<div class="row align-items-center">
						<div class="form-group validated col-sm-5 col-lg-5 col-12">
							...
							<select class="form-control select2" style="width: 100%" id="kt_select2_2">
								<option value="0">Selecione adicional</option>
								@foreach($adicionais as $a)
								<option value="{{$a->id}}">{{$a->id}} - {{$a->nome}} - R$ {{moeda($a->valor)}}</option>
								@endforeach
							</select>
						</div>
					...
					<div class="row align-items-center">
						...
						<div class="form-group col-lg-2 col-md-2 col-sm-6 col-6">
							...
							<div class="">
								<div class="input-group">
									<input type="text" value="1" name="quantidade" class="form-control @if($errors->has('quantidade')) is-invalid @endif" id="quantidade"/>
						...
						<div class="form-group col-lg-2 col-md-2 col-sm-6 col-6">
							<label class="col-form-label">Entregue</label>
							<div class="">
								<div class="input-group">
									...
									<tr class="datatable-row" @if($i->status) style="background: #64ffda" @endif>
										...
										<td class="datatable-cell">
											<span class="codigo" style="width: 180px;">
												...
												@if(!$i->status)
												<a href="/pedidos/alterarStatus/{{$i->id}}" class="btn btn-success btn-sm">

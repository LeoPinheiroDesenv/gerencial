...
<header>
	<div class="headReport">
		...
		@else
		<img src="{{'data:image/png;base64,' . base64_encode(file_get_contents(@public_path('imgs/Owner.png')))}}" alt="Logo" class="mb-2">
		...
		<div class="row">
			<h4 style="text-align:center; margin-top: -70px;">Relatório de Contas a Pagar</h4>
		</div>
...
<body>
	...
	<thead>
		<tr>
			...
			<th width="10%" class="text-left">Juros</th>
			<th width="10%" class="text-left">Multas</th>
	...
	<tbody>
		...
		$somaJuros = $somaLinhaJuros = 0; 
		$somaMulta = $somaLinhaMulta = 0; 
		@endphp
		...
		<tr style="font-size:15px; background: #D7D7D7;">
			<td colspan="10">Vencimento: {{ \Carbon\Carbon::parse($c->data_vencimento)->format('d/m/Y') }}</td>
		</tr>
		@endif
		...
		<tr style="font-size:12px;">
			...
			<td class="text-left">{{ number_format($c->juros, 2, ',', '.') }}</td>
			<td class="text-left">{{ number_format($c->multa, 2, ',', '.') }}</td>
		...
		if($c->status){
			...
			$somaJuros += $c->juros;
			$somaLinhaJuros += $c->juros;
			$somaMulta += $c->multa;
			$somaLinhaMulta += $c->multa;
		}else{
	...
	<tfoot>
		...
		<tr style="font-size:14px;">
			<td colspan="3">
				Total Juros: <strong>R$ {{number_format($somaJuros, 2, ',', '.')}}</strong>
			</td>
			<td colspan="5">
				Total Multa: <strong>R$ {{number_format($somaMulta, 2, ',', '.')}}</strong>
			</td>
		</tr>
		...
		<tbody>
			<tr>
				...
				<td class="text-right">
					...
					@else
					<img src="{{'data:image/png;base64,' . base64_encode(file_get_contents(@public_path('imgs/Owner.png')))}}" alt="Logo" class="mr-3">
					@endif
				</td>
			</tr>
		</tbody>

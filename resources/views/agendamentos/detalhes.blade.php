...
<div class="card card-custom gutter-b @if(env('ANIMACAO')) animate__animated @endif animate__bounce">
	<div class="card-body">
		<div class="" id="kt_user_profile_aside" style="margin-left: 10px; margin-right: 10px;">
			...
			<h3>Total: <strong class="text-info">{{moeda($agendamento->total)}}</strong></h3>
			@if($agendamento->desconto > 0)
			<h3>Desconto: <strong class="text-danger">{{moeda($agendamento->desconto)}}</strong></h3>
			@endif
			@if($agendamento->acrescimo > 0)
			<h3>Acrescimo: <strong class="text-danger">{{moeda($agendamento->acrescimo)}}</strong></h3>
			@endif
				...
				<div class="col-sm-6 col-lg-6 col-12">
					<div class="card card-custom gutter-b">
						<div class="card-body">
							...
							<p>{{$s->servico->nome}} x R$ {{ moeda($s->servico->valor) }}</p>
							@endforeach
						</div>
					</div>
				</div>
			...
			<div class="row">
				...
				<a class="btn btn-warning" href="/agendamentos/edit/{{ $agendamento->id }}">
					Editar agendamento			
				</a>

				<a style="margin-left: 10px;" class="btn btn-danger" onclick='swal("Atenção!", "Deseja remover este registro?", "warning").then((sim) => {if(sim){ location.href="/agendamentos/delete/{{ $agendamento->id }}" }else{return false} })' href="#!">
					Remover agendamento			
				</a>
				...
				<a style="margin-left: 10px;" class="btn btn-dark" onclick='swal("Atenção!", "Deseja alterar para finalizado?", "warning").then((sim) => {if(sim){ location.href="/agendamentos/alterarStatus/{{ $agendamento->id }}" }else{return false} })' href="#!">

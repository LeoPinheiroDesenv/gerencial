									...
									<div class="form-group col-lg-3 col-md-4 col-sm-6">
										...
										<div class="">
											<div class="input-group">
												<select disabled class="custom-select form-control" id="vendedor_id" name="vendedor_id">
													<option value="">Selecione </option>
													@foreach($vendedores as $v)
													<option @if($v->id == $venda->vendedor_id) selected @endif value="{{$v->id}}">{{$v->funcionario->nome}}</option>
													@endforeach
												</select>
												<button type="button" onclick="alterarVendedor()" class="btn btn-dark btn-sm">
													Alterar vendedor
												</button>
											</div>
										</div>

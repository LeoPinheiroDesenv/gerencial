...
<body>
	<div class="content">
				...
				<td class="" style="width: 760px;">
					<center>
						<label class="sub-titulo">
							ORÇAMENTO Nº: {{ $orcamento->numero_sequencial }} - EMISSÃO {{ date('d/m/Y') }}
							@if($orcamento->data_entrega)
							{{ \Carbon\Carbon::parse($orcamento->data_entrega)->format('d/m/Y') }}
							@endif
						</label>
					</center>
				</td>
		...
		<table class="div-first" style="border: 1px solid #555; border-radius: 10px;">
			...
			<tr>
				<td class="bold" style="width: 555px;">
					{{ $orcamento->cliente->rua }}, {{ $orcamento->cliente->numero }} {{ $orcamento->cliente->bairro }} - {{ $orcamento->cliente->cep }}
				</td>
				<td class="bold" style="width: 200px;">
					{{ $orcamento->cliente->cidade->info }}
				</td>
			</tr>
			@if($orcamento->cliente->complemento)
			<tr>
				<td class="bold" style="width: 555px;">
					{{ $orcamento->cliente->complemento }}
				</td>
			</tr>
			@endif
			<tr>
		...
		<table class="div-first" style="border: 1px solid #555; border-radius: 10px;">
			...
			<tr>
				...
				<td class="bold">{{number_format($i->quantidade * $i->valor, $casasDecimais, ',', '.')}}</td>
			</tr>
		...
		<table class="div-first" style="border: 1px solid #555; border-radius: 10px;">
			...
			<tr>
				<td class="" style="width: 250px;">
					Forma de pagamento: <strong> 
						...
						@if($orcamento->getFormaPagamento($orcamento->empresa_id) != null)
						- <span style="color: #8950FC">{{ $orcamento->getFormaPagamento($orcamento->empresa_id)->infos }}</span>
						@endif
					</strong>
				</td>
				<td class="" style="width: 250px;">
					Vendedor: 
					<strong>
						@if($orcamento->vendedor_setado && $orcamento->vendedor_setado->funcionario)
						{{ $orcamento->vendedor_setado->funcionario->nome }}
						@else
			...
			<tr>
				<td>
					Data da venda: <strong>{{\Carbon\Carbon::parse($orcamento->created_at)->format('d/m/Y H:i')}}</strong>
				</td>
				...
				<td style="font-size: 16px;">
					VALOR LIQUIDO: <strong style="color: red; font-size: 16px; margin-left: -20px;"> R${{ number_format($orcamento->valor_total - $orcamento->desconto + $orcamento->acrescimo, $casasDecimais, ',', '.') }}
					</strong>
				</td>
			</tr>
		...
		<table class="div-first" style="border: 1px solid #555; border-radius: 10px;">
			...
			<tr>
				...
				<td>
					<strong>{{number_format($d->valor, $casasDecimais, ',', '.')}}</strong>
				</td>
				<td>
